      ...
      return points
        .map((point, idx) => {
          const keyframeDragMovement =
            keyframeMovement && point.keyframeId === this.draggedKeyframeId ? keyframeMovement : NO_TRANSFORM
    ...
    private _renderColorKeyframes(keyframes: ReadonlyArray<Delir.Entity.Keyframe>) {
      ...
      return orderedKeyframes.map((kf, idx) => {
        ...
        const nextX = orderedKeyframes[idx + 1]
          ...
          : null
        const transform =
          this.state.keyframeMovement && kf.id === this.draggedKeyframeId ? this.state.keyframeMovement : { x: 0 }

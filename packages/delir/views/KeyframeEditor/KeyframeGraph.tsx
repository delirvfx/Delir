            ...
            process: {
                if (this.draggedKeyframeId) {
                    ...
                    if (!this._keyframeDragged) {
                        this.setState({
                            activeKeyframeId: this.draggedKeyframeId,
                            keyframeMovement: null,
                        })
                        break process
                    }
                    ...
                    const transitionPath = this.draggedEasingHandler.container.querySelector(
            ...
            return points
                .map((point, idx) => {
                    const keyframeDragMovement =
                        keyframeMovement && point.keyframeId === this.draggedKeyframeId
                            ? keyframeMovement
                            : NO_TRANSFORM
                    const easingHandleDragMovement =
                        easingHandleMovement && point.keyframeId === this.draggedEasingHandler!.keyframeId
                            ? easingHandleMovement
                            : NO_TRANSFORM
                    const easeOutHandleDragMovement =
                        selectedEasingHandler && selectedEasingHandler!.type === 'ease-out'
                            ? easingHandleDragMovement
                            : NO_TRANSFORM
                    const easeIntHandleDragMovement =
                        selectedEasingHandler && selectedEasingHandler!.type === 'ease-in'
                            ? easingHandleDragMovement
                            : NO_TRANSFORM
                        ...
                        <g key={point.keyframeId} data-index={idx}>
                            {point.transitionPath && (
                                ...
                                C ${point.transitionPath.begin.handleX + easeOutHandleDragMovement.x} ${point
                                        .transitionPath.begin.handleY + easeOutHandleDragMovement.y}
                                  ${point.transitionPath.end.handleX + easeIntHandleDragMovement.x} ${point
                                        .transitionPath.end.handleY + easeIntHandleDragMovement.y}
                            ...
                            {point.nextEaseInLine && (
                                ...
                                L ${point.nextEaseInLine.endX + easeIntHandleDragMovement.x} ${point.nextEaseInLine
                                        .endY + easeIntHandleDragMovement.y}
                            `}
                                    data-ease-in-handle-path
                                />
                            )}
                            <g
                                transform={`translate(${point.point.x + keyframeDragMovement.x - 4} ${point.point.y -
                                    4})`}
        ...
        private _renderColorKeyframes(keyframes: ReadonlyArray<Delir.Entity.Keyframe>) {
            ...
            return orderedKeyframes.map((kf, idx) => {
                ...
                const nextX = orderedKeyframes[idx + 1]
                    ? clipPlacedPositionX + this.frameToPx(orderedKeyframes[idx + 1].frameOnClip)
                    : null
                const transform =
                    this.state.keyframeMovement && kf.id === this.draggedKeyframeId
                        ? this.state.keyframeMovement
                        : { x: 0 }
